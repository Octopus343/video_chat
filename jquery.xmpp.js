    ...
    jQuery.xmpp ={
        ...
        */
        connect: function(options){
            ...
            jQuery.post(this.url,msg,function(data){
                var response = jQuery(xmpp.fixBody(data));
        ...
        */
        loginPlain: function(options){
            ...
            var url = this.url;
            jQuery.post(this.url,text,function(data){
                var response = jQuery(xmpp.fixBody(data));
                ...
                if(response.find("success").length){
                    ...
                    jQuery.post(url,text,function(data){
                        //xmpp.messageHandler(data);
                        xmpp.rid++;
         ...
         */
         __networkError: function(){
             ...
             if(jQuery.xmpp.onError != null){
                 jQuery.xmpp.onError({error:"'Network error'"})
             }
             
             if(jQuery.xmpp.onDisconnect != null){
                 jQuery.xmpp.onDisconnect()
             }
             
             jQuery.xmpp.__lastAjaxRequest.abort();
             jQuery.xmpp.connections = jQuery.xmpp.connections - 1;
             jQuery.xmpp.listening = false;
        ...
        listen: function(){
            ...
            if(!this.listening){
                ...
                if(xmpp.connections === 0) {
                    ...
                    this.rid = this.rid+1;
                    xmpp.connections = xmpp.connections + 1;
                    xmpp.__lastAjaxRequest = jQuery.ajax({
                      type: "POST",
                      url: this.url,
                      ...
                      success: function(data){
                            xmpp.connections = xmpp.connections - 1;
                            xmpp.listening = false;
                            var body = jQuery(xmpp.fixBody(data));
        ...
        */
        sendCommand: function(rawCommand, callback){
            ...
            jQuery.post(self.url,command,function(data){
        ...
        */
        sendMessage: function(options, data, callback){
            ...
            var messageObj = jQuery("<obj><message type='chat' to='"+toJid+"' xmlns='jabber:client'>bodyCont</message></obj>");
        ...
        */
        ...
        setSubscribition: function(to, state){
        var strop = "<presence to='"+to+"' type='"+state+"'/>";
        this.sendCommand(strop);
       },
        ...
        */
        getRoster: function(callback){
            ...
            var msg = "<iq><query xmlns='jabber:iq:roster'/></iq>";
            this.sendCommand(msg,function(data){
                var roster = [];
                jQuery.each(jQuery(data).find("item"), function(i,item){
        ...
        */
        ...
        messageHandler: function(data, context){
            var xmpp = this;
            var response = jQuery(xmpp.fixBody(data));
            jQuery.each(response.find("message"),function(i,element){
                try{
                    var e = jQuery(element);
                    xmpp.onMessage({from: e.attr("from"),to: e.attr('to'), body: e.find(".body").html(),attributes:e[0].attributes,data:response.children()});
                }catch(e){}
            });

            jQuery.each(response.find("iq"),function(i,element){
                if(xmpp.onIq){
                    var e = jQuery(element);
            ...
            jQuery.each(response.find("presence"),function(i,element){
                try{
                    var e = jQuery(element);
                    var status;
                    if(e.attr("type") != null){
